...
#define PANTILT_BUFFER_SIZE     (256)
...
static void camera_data_recv_entry(void* parameter)
{
    ...
    while (1)
    {
		...
		{
			szbuf += rt_device_read(dev1, 0, pbuf + szbuf, 1);
			continue;
		}else {
			if (szbuf == 0)
		...
		rt_device_write(dev5, 0, pbuf, szbuf);
...
//    rt_device_control(dev, RT_DEVICE_CTRL_CONFIG, &config);
	...
	pthread = rt_thread_create("tCAMrx", camera_data_recv_entry, env, 2048, 10, 20);
    RT_ASSERT(pthread != RT_NULL);
    result = rt_thread_startup(pthread);
    RT_ASSERT(result == RT_EOK);
