...
#define ZINGTO_PLANE_SIZE		(20)
#define ZINGTO_PLANE_HEADER		(0xEE)
#define ZINGTO_PLANE_HEADER1	(0x90)
...
void zingto_resolving_entry(void* parameter)
{
    ...
    while (1)
    {
        ...
        case 1:
			...
			{
				data_size=ZINGTO_PKT_SIZE;
			}
			else if(pbuf[0] == ZINGTO_PLANE_HEADER)
			{
				data_size=ZINGTO_PLANE_SIZE;
			}
			else
			{
				szbuf=0;
            ...
            break;
        default:
            szbuf += rt_device_read(dev, 0, pbuf + szbuf, data_size - szbuf);
            break;
        }
        ...
        if ((pbuf[1] != ZINGTO_PKT_HEADER1)&&(pbuf[1] != ZINGTO_PLANE_HEADER1))
            continue;
        
		if(pbuf[1] == ZINGTO_PLANE_HEADER1)
		{
			...
			env->dronelng=(float)dronelng_int32*0.0000001f;
			env->dronelat=(float)dronelat_int32*0.0000001f;
