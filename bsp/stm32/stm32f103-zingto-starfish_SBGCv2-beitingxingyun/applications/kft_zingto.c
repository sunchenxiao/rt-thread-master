...
#define ZINGTO_UARTPORT_NAME "uart2"
...
void zingto_resolving_entry(void* parameter)
{
	...
	dev4 = rt_device_find("uart4");
    RT_ASSERT(dev4 != RT_NULL);
    ...
    while (1)
    {
		...
		{
			rt_device_write(dev4, 0, pbuf, ZINGTO_PKT_SIZE);
		}
		else if(pbuf[2]==0x11&&pbuf[3]==0x01) //stop track
		{
			rt_device_write(dev4, 0, pbuf, ZINGTO_PKT_SIZE);
		}
		else if(pbuf[2]==0x11&&pbuf[3]==0x02) //start detection
		{
			rt_device_write(dev4, 0, pbuf, ZINGTO_PKT_SIZE);
		}
		else if(pbuf[2]==0x11&&pbuf[3]==0x03) //stop detection
		{
			rt_device_write(dev4, 0, pbuf, ZINGTO_PKT_SIZE);
		}
