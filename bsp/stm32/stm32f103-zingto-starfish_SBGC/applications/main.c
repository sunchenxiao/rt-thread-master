...
rt_uint8_t send_laser_data[7]={0xEF,0x00,0x00,0x00,0x00,0x00,0x00};
    ...
    while (RT_TRUE){
        ...
        if (result != -RT_ETIMEOUT) {
            ...
            if (szBuf == 5) {
				...
				if ( ( pBuf[0] == 0xE1) && (pBuf[1] == 0x1E) && (pBuf[2] == 0x15) && (pBuf[3] == 0xF1) && (pBuf[4] == 0x1F) ) {
...
ALIGN(RT_ALIGN_SIZE)
static rt_uint8_t uart2_rx_stack[ THREAD_STACK_SIZE ];
...
static void uart2_rx_entry(void* parameter)
{
    ...
    while (RT_TRUE){
		...
		if(szbuf>72){
			send_laser_data[3]=pbuf[69];
			send_laser_data[4]=pbuf[70];
			send_laser_data[5]=pbuf[71];
			send_laser_data[6]=pbuf[72];
		}
...
static void uart3_rx_entry(void* parameter)
{
    ...
    while (RT_TRUE){
		...
		if (szbuf == 10) {
			if ( ( pbuf[0] == 0xEE) && (pbuf[1] == 0x16) ) {
				
				send_laser_data[1]=pbuf[7];
				send_laser_data[2]=pbuf[6];
				szbuf=7;
				
				pMail = (struct PKT_MAIL *)rt_mp_alloc(mpUart2Tx, RT_WAITING_NO);
				if (pMail != RT_NULL) {
					pMail->size = szbuf;
					rt_memcpy(&pMail->payload[0], send_laser_data, szbuf);
					result = rt_mb_send(mbUart1Tx, (rt_ubase_t)pMail);

					if (RT_EOK != result) {
						rt_kprintf("%d, tRX1: mb send TX1 wrong\n", rt_tick_get());
						rt_mp_free(pMail);
						rt_thread_delay(1);
					}
				}
				else {
					rt_kprintf("%d, timeout to tx1, loop\n", rt_tick_get());
					while(1);
				}
