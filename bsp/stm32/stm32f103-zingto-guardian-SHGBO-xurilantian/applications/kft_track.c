...
//    rt_device_control(dev, RT_DEVICE_CTRL_CONFIG, &config);
    ...
    while (1)
    {
		...
		else if (env->trck_action == TRACK_ACTION_IRCOLOR)
        {
			...
			ctrlpkt.__reserved6=0x01;
			ctrlpkt.__reserved7=0x01;
			ctrlpkt.__reserved8[5]=0x00;
			ctrlpkt.set_offset_x=0x0000;
			ctrlpkt.set_offset_y=0x7A00;
			ctrlpkt.start_trace=0x44;
            if(env->irs_color == 0)
			{
