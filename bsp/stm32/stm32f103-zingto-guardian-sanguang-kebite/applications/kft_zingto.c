...
rt_uint8_t set_ptz_angle[20] = {0x3e,0x43,0x0f,0x52,0x02,0x02,0x05,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
...
void zingto_resolving_entry(void* parameter)
{
	...
	env->angle_control=RT_FALSE;
	env->speed_control=RT_TRUE;
    ...
    while (1)
    {
        ...
        if(pbuf[szbuf-5]==0xEE)
		{
			set_ptz_angle[13]=pbuf[szbuf-4];
			set_ptz_angle[14]=pbuf[szbuf-3];
			set_ptz_angle[17]=pbuf[szbuf-2];
			set_ptz_angle[18]=pbuf[szbuf-1];
			for(int i=4;i<19;i++)
			{
				set_ptz_angle[19]+=set_ptz_angle[i];
			}
			rt_device_write(dev2, 0, set_ptz_angle, 20);			
			set_ptz_angle[13]=0x00;
			set_ptz_angle[14]=0x00;
			set_ptz_angle[17]=0x00;
			set_ptz_angle[18]=0x00;
			set_ptz_angle[19]=0x00;		
			
			env->angle_control=RT_TRUE;
			env->speed_control=RT_FALSE;
			
		} 
		else if(pbuf[szbuf-5]==0xEF)
		{
			env->offset_x=pbuf[szbuf-4]+pbuf[szbuf-3]*256;
			env->offset_y=pbuf[szbuf-2]+pbuf[szbuf-1]*256;
			env->speed_control=RT_TRUE;
		} 
