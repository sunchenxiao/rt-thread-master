...
typedef struct __SHB30X_SendPacket
{
	...
	rt_int16_t		set_offset_x;       //b8
	rt_int16_t		set_offset_y;       //b10
...
static void track_data_recv_entry(void* parameter)
{
    ...
    pid_init(&pid_x, 1.0f, 0.02f, 0.5f);
    ...
    while (1)
    {
        ...
        if (env->trck_action == TRACK_ACTION_PREPARE)
        {
            ...
            ctrlpkt.set_fuction = 0xFF;
						...
						ctrlpkt.set_offset_x=env->offset_x;
						ctrlpkt.set_offset_y=env->offset_y;
        ...
        else if (env->trck_action == TRACK_ACTION_TRACE_START)
        {
			...
			ctrlpkt.set_trace_mode = 0x2C;		// medium size trace window.   // 0x3C: S,M,L     0x38: M,L    0x2C: S,M
