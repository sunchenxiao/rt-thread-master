...
rt_uint8_t ptz_target_location[27] = {0xEE,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
...
//			send_data[8]=env->cam_zoom_pos+1;
			...
			ptz_target_location[3]=pbuf[69];
			ptz_target_location[4]=pbuf[70];
			ptz_target_location[5]=pbuf[71];
			ptz_target_location[6]=pbuf[72];
			ptz_target_location[7]=env->droneyaw_L;
			ptz_target_location[8]=env->droneyaw_H;
			
			rt_device_write(dev5, 0, ptz_target_location, 27);
...
//			send_data[1]=pbuf[7];
			...
			factor_lng = calculateLineDistance(env->dronelat,env->dronelng,env->dronelat,(env->dronelng + 0.01f)) * 100.0f;
			float allyaw=env->droneyaw+env->ptz_yaw;
			
			float x = env->laser_dis*sin(PI / 180.0f * (90-env->ptz_pitch)) * sin(PI / 180.0f * allyaw);
			float y = env->laser_dis*sin(PI / 180.0f * (90-env->ptz_pitch)) * cos(PI / 180.0f * allyaw);
			locationlat = env->dronelat + y * 180.0f / (6371000.0f * PI);
			locationlng = env->dronelng + x / factor_lng;
			locationalt= env->dronealt-env->laser_dis*cos(PI / 180.0f * (90-env->ptz_pitch));
			...
			rt_int16_t dronealt_int16=(rt_int16_t)locationalt;
			rt_int32_t dronelng_int32=(rt_int32_t)(env->dronelng*10000000.0f);
			rt_int32_t dronelat_int32=(rt_int32_t)(env->dronelat*10000000.0f);
			
			ptz_target_location[1]=(rt_int16_t)env->laser_dis%256;
			ptz_target_location[2]=(rt_int16_t)env->laser_dis/256;

			rt_memcpy(&ptz_target_location[9],&locationlat_int32,4);
			rt_memcpy(&ptz_target_location[13],&locationlng_int32,4);
			ptz_target_location[17]=dronealt_int16%256;
			ptz_target_location[18]=dronealt_int16/256;
			rt_memcpy(&ptz_target_location[19],&dronelat_int32,4);
			rt_memcpy(&ptz_target_location[23],&dronelng_int32,4);
...
void ask_resolving_entry(void* parameter)
{
	...
	while(RT_TRUE)
	{
		...
		//rt_kprintf("droneyaw %d    dronealt %d    dronelat %d    dronelng %d \n",(rt_uint16_t)env->droneyaw,(rt_uint16_t)env->dronealt,(rt_uint16_t)env->dronelat,(rt_uint16_t)env->dronelng);
