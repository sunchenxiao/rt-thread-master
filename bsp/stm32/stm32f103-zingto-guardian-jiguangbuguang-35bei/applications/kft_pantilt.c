...
void pantilt_resolving_entry(void* parameter)
{
    ...
    while (1)
    {
		...
		else if (env->ptz_action == PANTILT_ACTION_SETLASER)
        {
			...
			{
				ptz_set_laser[4]=0x1c;
				ptz_set_laser[5]=0x20;
			}
			else if(env->cam_zoom_pos<2)
			{
				ptz_set_laser[4]=0x16;
				ptz_set_laser[5]=0x44;
			}
			else if(env->cam_zoom_pos<3)
			{
				ptz_set_laser[4]=0x13;
				ptz_set_laser[5]=0x24;
			}
			else if(env->cam_zoom_pos<4)
			{
				ptz_set_laser[4]=0x12;
				ptz_set_laser[5]=0x5c;
			}
			else if(env->cam_zoom_pos<5)
			{
				ptz_set_laser[4]=0x11;
				ptz_set_laser[5]=0x30;
			}
			else if(env->cam_zoom_pos<6)
			{
				ptz_set_laser[4]=0x10;
				ptz_set_laser[5]=0xcc;
			}
			else if(env->cam_zoom_pos<7)
			{
				ptz_set_laser[4]=0x0f;
				ptz_set_laser[5]=0x3c;
			}
			else if(env->cam_zoom_pos<8)
			{
				ptz_set_laser[4]=0x0e;
				ptz_set_laser[5]=0x74;
			}
			else if(env->cam_zoom_pos<10)
			{
				ptz_set_laser[4]=0x0c;
				ptz_set_laser[5]=0xe4;
			}
			else if(env->cam_zoom_pos<12)
			{
				ptz_set_laser[4]=0x0b;
				ptz_set_laser[5]=0xb8;
			}
			...
			{
				ptz_set_laser[4]=0x0a;
				ptz_set_laser[5]=0xf0;
			}
			else if(env->cam_zoom_pos<22)
			{
				ptz_set_laser[4]=0x0a;
				ptz_set_laser[5]=0x28;
			}
			else
			{
				ptz_set_laser[4]=0x08;
				ptz_set_laser[5]=0x98;
			}
...
void ask_resolving_entry(void* parameter)
{
	...
	while(RT_TRUE)
	{
		if(ask_laser_distance==0)
		{
			...
			{
				env->send_flag=0;
				rt_event_send(env->ev_camera, CAMERA_CMD_BRIGHT_GETPOS);
			}
		}else{
			ask_laser_distance=0;
		}
